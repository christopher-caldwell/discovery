...
Experiments use a copied baseline and macOS Seatbelt with filesystem reads, copy-only writes, and no network. Processes may signal their own children for worker cleanup; unrelated process signalling remains denied. New receipts retain the exact sandbox profile, including during same-version development. There is no unsandboxed fallback, credential-read isolation, or production database support. Reservation and receipt registration are separately audited transactions around execution; interrupted attempts never rerun implicitly. Captured streams use bounded in-memory pipes; exceeding 2,000,000 bytes per stream stops the process group and records an unsuccessful result. Project log filenames are preserved. This is not a quota for files written by the command. A killed controller can leave its child running; recovery requires inspecting and stopping the surviving attempt before explicit abort/replacement. Existing source bytes remain untouched. Source scans are not atomic snapshots of a concurrently changing filesystem.
